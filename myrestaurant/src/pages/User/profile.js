...
function Profile(props) {
    ...
    var getUser = function () {
        props.userAPI.get("".concat(endpoints["profile"]).concat(sessionStorage.getItem("username"), "/")).then(function (response) {
    ...
    var patchUser = function (data) {
        props.userAPI.patch("".concat(endpoints["profile"]).concat(sessionStorage.getItem("username"), "/"), data).then(function () {
            getUser();
        }).catch(function (error) { return setAPIFeedback(errorFormatter(error)); });
    };
    return (_jsxs(Container, __assign({ className: "profile ".concat(theme) }, { children: [_jsx(Row, __assign({ className: "title" }, { children: _jsx("h2", { children: "My Profile" }) })), _jsx(Row, __assign({ className: "error" }, { children: _jsx("ul", { children: APIFeedback.map(function (item, i) { return _jsx("li", { children: item }, i); }) }) })), _jsxs(Row, __assign({ xs: 1, sm: 2 }, { children: [_jsx(Col, __assign({ className: "profile-image" }, { children: _jsx("img", { src: Placeholder }) })), _jsx(Col, { children: _jsx(Table, __assign({ className: "profile-details" }, { children: _jsxs("tbody", { children: [_jsxs("tr", { children: [_jsx("td", __assign({ className: "key" }, { children: "Username" })), _jsx("td", { children: user.username })] }), _jsxs("tr", { children: [_jsx("td", __assign({ className: "key" }, { children: "Staff" })), _jsx("td", { children: _jsx(Form.Check, { type: "checkbox", label: "Staff View", name: "is_staff", checked: user.is_staff, onChange: function (e) { return patchUser({ is_staff: e.target.checked }); } }) })] }), user.is_staff &&
