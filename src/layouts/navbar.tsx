      ...
      <Navbar.Collapse className="nav-links" role="navigation">
        <Nav>
          <Nav.Link
            eventKey="1"
            as={NavLink}
            to={internalEndpoints.menu!}
            className="link"
          >
            Menu
          </Nav.Link>

          {user.isStaff && ["MANAGER", "SALES"].includes(user.role) && (
            <Nav.Link
              eventKey="2"
              as={NavLink}
              to={internalEndpoints.dashboard!}
              className="link"
            >
              Dashboard
            </Nav.Link>
          ...
          {user.isStaff && ["MANAGER", "SALES"].includes(user.role) && (
            <Nav.Link
              eventKey="3"
              as={NavLink}
              to={internalEndpoints.orders!}
              className="link"
            >
              Orders
            </Nav.Link>
          )}
          {user.isStaff && ["MANAGER", "CHEF"].includes(user.role) && (
            <Nav.Link
              eventKey="4"
              as={NavLink}
              to={internalEndpoints.inventory!}
              className="link"
            >
              Inventory
            </Nav.Link>
