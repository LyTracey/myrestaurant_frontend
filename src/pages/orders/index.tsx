...
function Orders() {
  ...
  const { EditIcon, DeleteIcon, FalseIcon, TrueIcon, CreateIcon, ArchiveIcon } =
    ICONS
  ...
  const filteredOrders = useMemo<{ [key: number]: string }>(
    () =>
      Object.fromEntries(
        orders.map((orderObj: { [key: string]: any }) => [
          orderObj.id,
          orderObj,
        ])
      ),
  ...
  const filteredMenu = useMemo<{ [key: number]: string }>(() => {
    return Object.fromEntries(
      menu.map((menuObj: MenuType) => [menuObj.id!, menuObj.title!])
    )
  }, [menu])
  ...
  const availabilities = useMemo<{ [key: number]: number }>(
    () =>
      Object.fromEntries(
        menu.map((item: any) => [item.id, item.available_quantity])
      ),
    [menu]
  )
  ...
  const Columns: GridColDef[] = [
            ...
            {value.map((menuID: number, i: number) => {
              return (
                <div className="menu-item" key={`menu_item_${i}`}>
                  <span>{`${filteredMenu[menuID]}`}</span>{" "}
                  <span>x {row.quantity[menuID]}</span>
                </div>
              )
            })}
          </div>
    ...
    },
    {
      ...
      type: "dateTime",
      width: 175,
      valueGetter: ({ row }) => new Date(row.ordered_at),
    },
    {
      field: "prepared",
      headerName: "Prepared",
      ...
      renderCell: ({ row }: GridRenderCellParams) => (
        <Tooltip
          title={
            row.prepared ? `Prepared at ${row.prepared_at}` : "Not prepared"
          }
          placement="right-end"
        >
    ...
    {
      field: "prepared_at",
      headerName: "Prepared Time",
      type: "dateTime",
      width: 175,
      valueGetter: ({ row }) =>
        row.prepared_at ? new Date(row.prepared_at) : null,
    },
    {
      field: "delivered",
      ...
      renderCell: ({ row }: GridRenderCellParams) => (
        <Tooltip
          title={
            row.delivered ? `Delivered at ${row.delivered_at}` : "Not delivered"
          }
          placement="right-end"
        >
          <div onClick={(e) => handleCheck(e, row, "delivered")}>
            {row.delivered ? <TrueIcon /> : <FalseIcon />}
          </div>
        </Tooltip>
      ),
    },
    ...
    },
    {
      ...
      renderCell: ({ row }: GridRenderCellParams) => (
        <Tooltip
          title={row.complete ? `Order complete!` : "Order not complete"}
          placement="right-end"
        >
    ...
    },
    {
      ...
      getActions: (params: GridRowParams) => [
        <GridActionsCellItem
          icon={<EditIcon title="Edit order" className="table-button" />}
          onClick={() => {
        ...
        />,
        <GridActionsCellItem
          icon={<DeleteIcon title="Delete order" className="table-button" />}
          onClick={() => {
    ...
    <Container className={`page orders ${theme}`}>
      ...
      <Row xs={2} className="actions">
        <Button
          ...
          onClick={() => navigate(internalEndpoints.ordersCreate!)}
        >
          <CreateIcon className="icon" />
        </Button>
        <Button className="button archive" as="a" href="/orders/archive">
        ...
        <DeleteAlert
          onClickYes={async () => {
            await dataAPI.delete(
              `${externalEndpoints.orders!}${deleteObj.current}`
            )
            setShowDelete(false)
            revalidator.revalidate()
          }}
