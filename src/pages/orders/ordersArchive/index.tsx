...
function ArchivedOrders() {
  ...
  const handleEditOrder = async (e: MouseEvent<HTMLElement>, id: number) => {
    ...
    await dataAPI.patch(`${externalEndpoints.orders}${id}/`, {
      complete: false,
    })
    ...
    },
    {
      ...
      renderCell: ({ row }: GridRenderCellParams) => (
        <Tooltip
          ...
          placement="right-end"
        >
          <div>{row.complete ? <TrueIcon /> : <FalseIcon />}</div>
        </Tooltip>
      ),
    },
    {
      ...
      getActions: ({ row }: GridRowParams) => [
        <GridActionsCellItem
          icon={<EditIcon className="table-button" />}
          onClick={(e: MouseEvent<HTMLElement>) => handleEditOrder(e, row.id)}
          label="Edit"
        />,
      ],
    },
