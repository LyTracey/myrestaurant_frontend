...
import { Statistics, StatisticsObj, DateRange } from "./dashboard.types"
...
function Dashboard() {
  ...
  useEffect(() => {
    dataAPI
      .patch(`${externalEndpoints["dashboard"]}`, dateRange)
  ...
  const createAccordion = (
    status: string,
    title: string,
    statistic: Array<string> | StatisticsObj,
    comment: string
  ) => {
    return (
        ...
        <Accordion.Item eventKey="0">
          <Accordion.Header className={status}>
            <div className="header">
              {title}{" "}
              <Badge>{statistic.length ?? Object.keys(statistic).length}</Badge>
            </div>
          </Accordion.Header>
          <Accordion.Body>
